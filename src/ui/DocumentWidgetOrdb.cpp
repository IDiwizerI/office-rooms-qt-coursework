...
constexpr int RequiredColumnCount = 12;
const QString ColumnSettingsKey = QStringLiteral("roomsTable");
const QString DatabaseSignature = QStringLiteral("OFFICE_ROOMS_DB_V1");
const QString DatabaseHeader = QStringLiteral("id;building;floor;roomNumber;roomType;area;capacity;department;responsiblePerson;condition;lastRenovationDate;notes");
...
Room roomFromFields(const QStringList &fields)
{
    ...
    room.roomNumber = fields.value(RoomTableModel::RoomNumberColumn).trimmed();
    room.roomType = fields.value(RoomTableModel::RoomTypeColumn).trimmed();
    room.area = fields.value(RoomTableModel::AreaColumn).toDouble();
    room.capacity = fields.value(RoomTableModel::CapacityColumn).toInt();
    room.department = fields.value(RoomTableModel::DepartmentColumn).trimmed();
    room.responsiblePerson = fields.value(RoomTableModel::ResponsiblePersonColumn).trimmed();
    room.condition = fields.value(RoomTableModel::ConditionColumn).trimmed();
    room.lastRenovationDate = dateFromString(fields.value(RoomTableModel::LastRenovationDateColumn));
    room.notes = fields.value(RoomTableModel::NotesColumn).trimmed();
    return room;
}
...
QString roomToLine(const Room &room)
{
    return QStringList{
        ...
        escapeField(room.roomNumber),
        escapeField(room.roomType),
        QString::number(room.area, 'f', 2),
        QString::number(room.capacity),
        escapeField(room.department),
        escapeField(room.responsiblePerson),
        escapeField(room.condition),
        room.lastRenovationDate.toString(DateFormat),
        escapeField(room.notes)
    }.join(';');
}
} // namespace
...
QString DocumentWidget::filePath() const { return m_filePath; }
...
bool DocumentWidget::isModified() const { return m_modified; }
QTableView *DocumentWidget::tableView() const { return ui->roomsTableView; }

bool DocumentWidget::loadFromFile(const QString &filePath, QString *errorMessage)
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        if (errorMessage != nullptr) { *errorMessage = tr("Cannot open file for reading."); }
        return false;
    }
    ...
    if (input.readLine().trimmed() != DatabaseSignature) {
        if (errorMessage != nullptr) { *errorMessage = tr("The selected file is not an office rooms database."); }
        return false;
    }

    if (input.readLine().trimmed() != DatabaseHeader) {
        if (errorMessage != nullptr) { *errorMessage = tr("The database header is damaged."); }
        return false;
    }
    ...
    while (!input.atEnd()) {
        ...
        if (fields.size() != RequiredColumnCount) {
            if (errorMessage != nullptr) { *errorMessage = tr("Invalid field count at line %1.").arg(lineNumber); }
            return false;
        }
...
bool DocumentWidget::save(QString *errorMessage)
{
    if (m_filePath.isEmpty()) {
        if (errorMessage != nullptr) { *errorMessage = tr("File path is empty. Use Save As."); }
        return false;
    }
...
bool DocumentWidget::saveAs(const QString &filePath, QString *errorMessage)
{
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Truncate)) {
        if (errorMessage != nullptr) { *errorMessage = tr("Cannot open file for writing."); }
        return false;
    }

    QTextStream output(&file);
    output.setEncoding(QStringConverter::Utf8);
    output << DatabaseSignature << '\n';
    output << DatabaseHeader << '\n';
    for (const Room &room : m_model->rooms()) { output << roomToLine(room) << '\n'; }
...
void DocumentWidget::saveColumnWidths() const { SettingsManager().saveColumnWidths(ui->roomsTableView, ColumnSettingsKey); }
void DocumentWidget::restoreColumnWidths() { SettingsManager().restoreColumnWidths(ui->roomsTableView, ColumnSettingsKey); }

void DocumentWidget::addRoom()
{
    RoomEditDialog dialog(this);
    dialog.setWindowTitle(tr("Add room"));
    if (dialog.exec() == QDialog::Accepted) { m_model->addRoom(dialog.room()); }
}
...
void DocumentWidget::editSelectedRoom()
{
    ...
    if (dialog.exec() == QDialog::Accepted) { m_model->updateRoom(sourceRow, dialog.room()); }
}
...
void DocumentWidget::deleteSelectedRooms()
{
    ...
    if (QMessageBox::question(this, tr("Delete rooms"), tr("Delete selected rooms?")) != QMessageBox::Yes) { return; }
    QList<int> sourceRows;
    for (const QModelIndex &proxyIndex : selectedRows) { sourceRows.append(m_proxyModel->mapToSource(proxyIndex).row()); }
    std::sort(sourceRows.begin(), sourceRows.end(), std::greater<int>());
    for (const int row : sourceRows) { m_model->removeRoom(row); }
}
...
{
    if (index.isValid()) { editSelectedRoom(); }
}

void DocumentWidget::markModified()
{
    if (!m_loading) { setModified(true); }
}
